...
If a hash is provided and one of the options name is <strong>'attributes'</strong>, this represents a "Raphael attribute":http://raphaeljs.com/reference.html#attr. The tables bellow define only the subset of default options for Raphael attributes but all valid Raphael attributes can be used.
