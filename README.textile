...
new Ico.LineGraph(
  $( 'gross_profit_id' ),                             // div where the graph will be rendered
  {                                                   // The 2 series
...
Graph features are driven by components. Each graph type defines different default components and graph orientation (horizontal or vertical) and other graph-specific options. Default components can be disabled by explicitly setting their option to false.
...
<table>
  ...
  <tr>
    ...
    <td>Ico.Component<td>
    <td>mouse_pointer</td>
    <td>Display the mouse pointer as cross when in graph area</td>
  </tr>
  <tr>
...
h3. Component.Labels Options

The option attribute 'labels' defines values labels. If an array is provided, it reprensent the values of the labels. If more label options are provided, labels must be provided as a hash of options defined in the following table:
...
h3. Component.ValueLabels Options

Values labels are always calculated from the range of data series values. Additional options can be provided in the value_labels attribute as a hash:
...
h3. Component.Graph (background) options
...
h3. Component.Background options
...
h3. Component.Meanline options

If the meanline attribute option is set to true or a hash of additional attributes, Ico calculates the mean of all values of all series and displays this mean value as line along the labels axis.
...
h3. Component.MousePointer options
...
h3. Component.StatusBar options
